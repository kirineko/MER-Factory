...
如果你准备使用 `--huggingface-model`，或者当前环境不方便在首次运行时联网拉取模型，建议先把需要的模型下载到 Hugging Face 默认缓存目录。
...
请使用新的 `hf download` 命令，不要再使用旧的 `huggingface-cli download`，也不需要传 `--local-dir`。

当前仓库注册的 Hugging Face 推理模型包括：
...
这些命令默认下载到 Hugging Face 缓存目录，通常是 `~/.cache/huggingface/`，后续运行会直接复用。
